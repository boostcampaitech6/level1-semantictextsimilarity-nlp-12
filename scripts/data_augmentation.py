...
def data_augmentation(path, ratio=1):
    ...
    for src_idx, src_row in tqdm(data.iterrows(), desc='data_augmentation', total=len(data)):
        ...
        for trg_idx, trg_row in data.sample(n=1).iterrows():
            if src_idx == trg_idx:
                continue
            aug_rows.append([f'{src_idx}_{trg_idx}', 'augmented', src_row['sentence_1'], trg_row['sentence_2'], 0.3, 0])
    ...
    data.to_csv('./train_augmented_1_0.3.csv')
